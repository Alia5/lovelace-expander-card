...
<script lang="ts">
  ...
  onMount(() => {
      isEditorMode =
      (thisComponent as HTMLElement).parentElement?.localName ===
      'hui-card-preview';
      if (isEditorMode) {
          expanded = true;
      } else {
...
>
  {#if config['title-card']}
    ...
    >
      ...
      <button
        style="--overlay-margin:{config[
            'overlay-margin'
        ]}; --button-background:{config['button-background']};"
...
<style>
  ...
  .ripple:hover {
    background: #ffffff12 radial-gradient(circle, transparent 1%, #ffffff12 1%)
      center/15000%;
  }
  .ripple:active {
